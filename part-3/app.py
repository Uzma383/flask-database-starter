...
@app.route('/')
def index():
    students = Student.query.order_by(Student.name).all()   # ORDER BY example
    return render_template('index.html', students=students)
...
def init_db():
    with app.app_context():
        ...
        if Course.query.count() == 0:
            courses = [
                Course(name='Python Basics', description='Learn Python fundamentals', teacher_id=1),
                Course(name='Web Development', description='HTML, CSS, Flask', teacher_id=2),
                Course(name='Data Science', description='Data analysis with Python', teacher_id=1),
            ]
